...
Yii::import('extensions.giix-core.helpers.*');
...
class GiixCrudCode extends CrudCode {
	...
	public function generateActiveField($modelClass, $column) {
		$allowNull = isset($column->allowNull) && $column->allowNull ? true : false;
		
		if ($column->isForeignKey) {
			$relation = $this->findRelation($modelClass, $column);
			$relatedModelClass = $relation[3];
			return "echo \$form->dropDownList(\$model, '{$column->name}', GxHtml::listDataEx({$relatedModelClass}::model()->findAll()), array('class' => 'form-control'".($allowNull ? ", 'empty' => '---'" : "")."))";
		}
	
		$spl = preg_split('/(?=[A-Z])/', $modelClass, -1, PREG_SPLIT_NO_EMPTY);
		...
		if ($this->isCheckbox($column)) {
			...
			return "echo \$form->textArea(\$model, '{$column->name}', array('class' => 'form-control autogrow'))";
		} else {
			...
			if ($column->type !== 'string' || $column->size === null)
				...
				return "echo \$form->{$inputField}(\$model, '{$column->name}', array('class' => 'form-control', 'maxlength' => {$column->size}))";
		...
		if (!$column->isForeignKey) {
			...
			return "array(
				...
				'value' => 'GxHtml::valueEx(\$data->{$relationName})',
                                'filter' => GxHtml::listDataEx({$relatedModelClass}::model()->findAllByAttributes(array(), array('order' =>{$relatedModelClass}::representingColumn()))),
...
EOM;
	...
	public function hasColumnThatEndsWith($suffix)
	{
		$spl = preg_split('/(?=[A-Z])/', $this->modelClass, -1, PREG_SPLIT_NO_EMPTY);
		$underlined = strtolower(implode('_', $spl));
		return isset($this->tableSchema->columns[$underlined . $suffix]);
	}
	...
	public function isCheckbox($column)
	{
		...
		return strtoupper($column->dbType) == 'TINYINT(1)'
				|| strtoupper($column->dbType) == 'TINYINT(3)'
				|| strtoupper($column->dbType) == 'TINYINT(4)'
				|| strtoupper($column->dbType) == 'BIT'
				|| strtoupper($column->dbType) == 'BOOL'
				|| strtoupper($column->dbType) == 'BOOLEAN'
				|| $column->name == ($underlined . '_active');
	}
}